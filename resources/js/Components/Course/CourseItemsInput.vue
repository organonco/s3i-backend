<template>
    ...
    <add-item-dialog title="إضافة محور" v-model="section.dialog" @save="saveDialog('section')" :index="index">
        ...
        <v-textarea label="الوصف" variant="solo" v-model="section.object.description" />
    </add-item-dialog>
    ...
    <add-item-dialog title="إضافة فيديو" v-model="video.dialog" @save="saveDialog('video')" :index="index">
        ...
        <v-textarea label="الوصف" variant="solo" v-model="video.object.description" />
    ...
    <add-item-dialog title="إضافة اجتماع" v-model="meeting.dialog" @save="saveDialog('meeting')" :index="index">
        ...
        <v-textarea label="الوصف" variant="solo" v-model="meeting.object.description" />
    </add-item-dialog>
    ...
    <add-item-dialog title="رفع ملف" v-model="file.dialog" @save="saveDialog('file')" :index="index">
        ...
        <v-textarea label="الوصف" variant="solo" v-model="file.object.description" />
    ...
    <add-item-dialog :title="quiz?.object?.type === 'text' ? 'إضافة وظيفة' : 'إضافة اختبار'" v-model="quiz.dialog"
        @save="saveDialog('quiz')" :index="index">
        <v-text-field :label="quiz?.object?.type === 'text' ? 'اسم الوظيفة' : 'اسم الاختبار'" variant="solo"
            v-model="quiz.object.name" :rules="requiredRule" />
        <v-textarea label="الوصف" variant="solo" v-model="quiz.object.description" />
        <v-text-field type="number" v-if="quiz?.object?.type === 'multiple_choice'" label="وقت الاختبار (بالدقائق)"
            variant="solo" v-model="quiz.object.time_limit" />
        <v-select style="display: none" label="نوع الاختبار" variant="solo" v-model="quiz.object.type"
            :rules="requiredRule" :items="quiz_types" item-title="name" item-value="value"
            :disabled="quiz.object.questions.length !== 0" />
        <quiz-questions-input v-if="quiz.object.type !== null" v-model="quiz.object.questions"
            :quiz_type="quiz.object.type"></quiz-questions-input>
        <quiz-preview v-if="quiz.object.questions.length > 0" :questions="quiz.object.questions"
            :title="quiz.object.name"></quiz-preview>
    </add-item-dialog>
</template>
