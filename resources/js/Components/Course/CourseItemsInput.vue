<template>
    ...
    <v-row justify="center" class="mb-2">
        <v-btn class="ma-1" color="primary" @click="openDialog(-1, 'section')">إضافة قسم</v-btn>
        ...
        <v-btn class="ma-1" color="primary" @click="openDialog(-1, 'file')">إضافة ملف</v-btn>
        <v-btn class="ma-1" color="primary" @click="openDialog(-1, 'homework')">إضافة وظيفة</v-btn>
        <v-btn class="ma-1" color="primary" @click="openDialog(-1, 'quiz')">إضافة اختبار</v-btn>
    </v-row>

    <add-item-dialog title="إضافة قسم" v-model="section.dialog" @save="saveDialog('section')" :index="index">
        <v-text-field label="اسم القسم" variant="solo" v-model="section.object.name" :rules="requiredRule"/>
    </add-item-dialog>
    ...
    <add-item-dialog title="إضافة ملف" v-model="file.dialog" @save="saveDialog('file')" :index="index">
    ...
    <add-item-dialog title="إضافة وظيفة" v-model="homework.dialog" @save="saveDialog('homework')" :index="index">
        <v-label>تتيح الوظيفة للطالب رفع ملف
        </v-label>
        <v-text-field label="عنوان الوظيفة" variant="solo" v-model="homework.object.name" :rules="requiredRule"/>
    </add-item-dialog>

    <add-item-dialog title="إضافة اختبار" v-model="quiz.dialog" @save="saveDialog('quiz')" :index="index">
        <v-text-field label="اسم الاختبار" variant="solo" v-model="quiz.object.name" :rules="requiredRule"/>
        <v-select label="نوع الاختبار" variant="solo" v-model="quiz.object.type" :rules="requiredRule" :items="quiz_types"
